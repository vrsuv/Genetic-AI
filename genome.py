...
# a gene would be an array of numbers (params)
# each gene converts into a link
# multiple links if it has higher recurrence
# genome is made up of multiple genes
...
class Genome():
    ...
    @staticmethod 
    # a genome with gene_count number of genes
    def get_random_genome(gene_length, gene_count):
        genome = [Genome.get_random_gene(gene_length) for i in range(gene_count)]
        return genome

    @staticmethod
    # genome spec = descriptor of genome (dictionary)
    # maps parameter names to scaling factors and gene positions
    def get_gene_spec():
        gene_spec =  { 
            "link-shape":{"scale":1}, 
            "link-length": {"scale":10},
            "link-radius": {"scale":3},
            "link-recurrence": {"scale":0},
            "link-mass": {"scale":1},
            "joint-type": {"scale":0},
            "joint-parent":{"scale":0},
        ...
        # assigning index for each param above
        ind = 0 #index
    ...
    @staticmethod
    # function to produce a dictionary of the 
    # long string of numbers for each gene
    def get_gene_dict(gene, spec):
        gdict = {}
        for key in spec:
            ind = spec[key]["ind"] # which element in the gene array you're using
            scale = spec[key]["scale"] 
            gdict[key] = gene[ind] * scale
        return gdict

    @staticmethod
    # function to combine gdict of each gene for genome
    ...
    @staticmethod
    # function to get like ['A', B1', 'C2', 'D3', 'C4', 'D5']
    def expandLinks(parent_link, uniq_parent_name, flat_links, exp_links):
        ...
        sibling_ind = 1
        for c in children:
            for r in range(int(c.recur)): # each recurrence the child has
                ...
                Genome.expandLinks(c, uniq_name, flat_links, exp_links) # get children for each children

    @staticmethod
    def genome_to_links(gdicts):
        link_ind = 0 # parent index
        parent_names = [str(link_ind)] # need to know parents name to know who you're connecting to
        links = []
        # adding all links to parent
        for gdict in gdicts:
            link_name = str(link_ind)
            parent_ind = gdict["joint-parent"] * len(parent_names) # get parent name from gene_spec
            ...
            links.append(link)
            if link_ind != 0: # don't re-add for the first link
    ...
    @staticmethod
    # taking first part of g1 and last part of g2 and concatenate
    def crossover(g1, g2):
        """
    ...
    @staticmethod
    # rate is per gene how likely it is to mutate that gene 
    # rate = 1 means every gene will get mutated
    # amount is where in that gene, how much it gets mutated by
    ...
    @staticmethod
    # function to randomly remove a gene from a genome
    # rate is how likely it is that one of the genes is going to be removed
    # rate = 1 means definitely a gene will be removed
...
class URDFLink:
    # contains all data to represent a link
    # get genome list, pass it through genome_spec, 
    # and extract out these URDF objects
    ...
    def to_link_element(self, adom):
        ...
        mat_tag = adom.createElement("material")
        mat_tag.setAttribute("name", self.name)
        col_tag = adom.createElement("color")

        col = (str(float(random.randint(0, 1))) + 
            " " + str(float(random.randint(0, 1))) + 
            " " + str(float(random.randint(0, 1))) + 
            " " + str(1.0))
        col_tag.setAttribute("rgba",  col)

        mat_tag.appendChild(col_tag)
        vis_tag.appendChild(mat_tag)
        ...
        # mass_tag.setAttribute("value", "0.25")
    ...
    def to_joint_element(self, adom):
        ...
        # limit_tag.setAttribute("upper", "-10")
        # limit_tag.setAttribute("lower", "10")
